...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: $0 [--out-dir DIRECTORY] [--plugin-name STEAMPIPE_PLUGIN] [--debug]"
            exit 1
            ;;
    esac
done
...
echo "------------------ ANY CSV  SCRIPT HERE-------------------------------"
# ======= LET THE DOGS OUT =======
log_debug "Script started with arguments: STEAMPIPE_PLUGIN_NAME=${STEAMPIPE_PLUGIN_NAME} OUT_DIR=$OUT_DIR, KDIFF_DEBUG=$KDIFF_DEBUG, SQL_LIMIT_STR=$SQL_LIMIT_STR"
