...
volumes:
  ...
  - name: steampipe-config-volume
    configMap:
      name: {{ include "kdiff-cli.fullname" . }}-steampipe-config
      optional: true
      items:
      {{- range $key, $value := .Values.steampipeConfig }}
      - key: {{ $key }}
  ...
  - name: steampipe-credentials-volume
    secret:
      secretName: {{ include "kdiff-cli.fullname" . }}-steampipe-creds
      optional: true
      items:
      {{- range $config := .Values.steampipeSecretCredentials }}
      - key: {{ $config.name }}
  ...
  - name: steampipe-initdb-volume
    configMap:
      name: {{ include "kdiff-cli.fullname" . }}-initdb-sql-files
      optional: true
      items:
      {{- range $key, $value := .Values.initDbSqlScripts }}
      - key: {{ $key }}
...
Common labels
*/}}
{{- define "kdiff-cli.labels" -}}
helm.sh/chart: {{ include "kdiff-cli.chart" . }}
{{ include "kdiff-cli.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
Selector labels
*/}}
{{- define "kdiff-cli.selectorLabels" -}}
app.kubernetes.io/name: {{ include "kdiff-cli.name" . }}
...
*/}}
{{- define "kdiff-cli.serviceAccountName" -}}
{{- default (include "kdiff-cli.fullname" .)  }}
{{- end }}
